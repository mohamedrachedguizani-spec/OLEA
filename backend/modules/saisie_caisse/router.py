...
def get_dashboard_stats(
    ...
    with db.get_cursor() as cursor:
        ...
        # Solde actuel (dernier solde cumulé sans filtre)
        cursor.execute("""
            SELECT solde 
            FROM ecritures_caisse 
        ...
        cursor.execute(f"""
            ...
            LIMIT 5
        """, params)
        top_libelles = cursor.fetchall()

        return {
            "solde_actuel": solde_actuel,
            "total_debit": float(stats['total_debit']) if stats['total_debit'] else 0,
            "total_credit": float(stats['total_credit']) if stats['total_credit'] else 0,
            ...
            "evolution": [
                {
                    "jour": str(e['jour']),
                    "debit": float(e['debit_jour']) if e['debit_jour'] else 0,
                    "credit": float(e['credit_jour']) if e['credit_jour'] else 0,
                }
                for e in evolution
            ],
            "top_libelles": [
                {
                    "libelle": t['libelle_ecriture'],
                    "occurrences": t['occurrences'],
                    "total_debit": float(t['total_debit']) if t['total_debit'] else 0,
                    "total_credit": float(t['total_credit']) if t['total_credit'] else 0,
                }
                for t in top_libelles
            ],
        }


# ═══════════════════════════════════════════════════════════
# 5. Nettoyage historique migré
# ═══════════════════════════════════════════════════════════
