...
function SaisieBancaire() {
    ...
    // --- Auto-sauvegarde debounced de la session (2s) ---
    useEffect(() => {
        if (!batch?.id || Object.keys(ligne2ByMovement).length === 0) return;
        if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
        saveTimerRef.current = setTimeout(async () => {
            try {
                await ApiService.saveSessionEntries(batch.id, ligne2ByMovement);
            } catch (err) {
    ...
    const handleResumeSession = useCallback(async (sessionBatch) => {
        ...
        try {
            ...
            setBatch(batchData);
            const items = await ApiService.getBankReconciliationMovements(sessionBatch.id);
            setMovements(Array.isArray(items) ? items : []);
            // Restaurer les entrées de session
            const entries = await ApiService.getSessionEntries(sessionBatch.id);
            if (Array.isArray(entries) && entries.length > 0) {
                const restored = {};
                entries.forEach((e) => {
                    restored[e.movement_id] = {
                        compte: e.compte || '',
                        tiers: e.tiers || '',
                        section_analytique: e.section_analytique || '',
                    };
                });
                setLigne2ByMovement(restored);
            }
            setPage(1);
            setStep(2);
            setMessage('Session restaurée avec succès.');
        } catch (err) {
            setError(err.message || 'Erreur lors de la restauration de la session.');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleFormChange = (e) => {
        ...
        if (name === 'compte_banque') {
            setFormData((prev) => ({
                ...
                taux_conversion: deviseComptes[value] ? prev.taux_conversion : '',
            }));
            return;
        }
    ...
    const handleUpload = async (e) => {
        ...
        if (needsTaux && (!formData.taux_conversion || Number(formData.taux_conversion) <= 0)) {
            setError(`Le compte ${formData.compte_banque} est en ${deviseComptes[formData.compte_banque]}. Veuillez saisir un taux de conversion valide.`);
            return;
        }
        ...
        if (needsTaux && formData.taux_conversion) {
            payload.append('taux_conversion', formData.taux_conversion);
        }
                    ...
                    <div className="form-col">
                        <div className="form-group">
                            ...
                            <input
                                ...
                                value={formData.taux_conversion}
                                onChange={handleFormChange}
                                className="form-control"
                                placeholder="Ex: 3.350"
                                step="0.001"
                                min="0.001"
                                required
                            />
